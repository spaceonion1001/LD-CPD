...
def average_AMOC_new(fprs, dts, debug_title="Ours", to_plot=True):
    os.makedirs('debugging_figs', exist_ok=True)
...
def main_sims(clime=False):
    ...
    sim_types = ['orthogonal_small', 'orthogonal_cross_block', 'orthogonal_multiple_block', 'orthogonal_hard', 'cai_model_one', 'cai_model_three']
    dims = [20, 40, 60, 80]
    post_window_size = 20 # fixed across all sims
    our_results = {}
    cai_results = {}
    kesh_results = {}
    kesh_alt_results = {}
    for curr_dim in dims:
        ...
        if curr_dim == 20:
            ...
            window_size = 200
        os.makedirs(curr_dim_path, exist_ok=True)
        os.makedirs(os.path.join(curr_dim_path, 'avg_amoc'), exist_ok=True)
...
def main_mesonet(storm_name='center', clime=False):
    ...
    os.makedirs(curr_storm_save_path, exist_ok=True)

    cpd_path = 'results/{}_storm'.format(storm_name)

    cpd_path = 'results/{}_storm_unprocessed'.format(storm_name)
    os.makedirs('debugging_figs/mesonet_center', exist_ok=True)
    ...
    for num in sorted(storm_nums):
        ...
        # the 12 is for 1 hour buffer before and after storm to account for sliding window edges
        # all algorithms have a fair comparison here
    ...
    for num in sorted(storm_nums):
        ...
        if storm_start.item() == storm_end.item():
            ...
            if curr_tuple in list(set(unique_tuples)):
                ...
                unique_tuples.append(curr_tuple)
        # 12 is for padding an hour before/after, to compensate for the sliding window edges and ensure the storm is fully contained in the window as it slides over the data
        # this applies to all algorithms, it's a fair comparison
